...
# worker_server.py (v27.2 - Dynamic Pipeline Handler - Complete & Verified)
import logging
import json
import os
import uuid
...
app = FastAPI(title="Morpheus AI Pod (Libertatem)", version="27.2")
...
def initialize_specs_and_benchmark():
    ...
    try:
        ...
        time.sleep(45)
        # La lógica del benchmark puede ser re-implementada aquí si es necesario
...
def run_chained_job_thread(client_id: str, task_chain: list):
    ...
    pipeline_context = {}; total_tasks = len(task_chain)
    try:
        for i, task in enumerate(task_chain):
            workflow_name = task['workflow']; config_payload = task.get('config_payload', {})
...
async def create_job(payload: JobPayload):
    ...
    if workflow_name == "full_transform":
        logger.info(f"[Orquestador] Recibido 'full_transform' ID: {client_id}")
        task_chain = [
            {"workflow": "extract_audio", "config_payload": {"source_video_pod_path": config["target_video_pod_path"]}},
            {"workflow": "voice_style_transfer", "config_payload": config},
            {"workflow": "video_transfer", "config_payload": config},
            {"workflow": "mux_video_audio"}
        ]
        if config.get("enable_voice_clean", True): task_chain.insert(1, {"workflow": "clean_audio"})
        if config.get("enable_post_process", False): task_chain.append({"workflow": "post_process_veritas", "config_payload": config})
        
        thread = threading.Thread(target=run_chained_job_thread, args=(client_id, task_chain))
        thread.start()
        return {"message": "Pipeline 'full_transform' orquestado", "id": client_id, "status": "IN_QUEUE"}
